...
let generators_extended() =
  try
    ...
    Printf.printf "test 0\n";errorer (s 1) (unsafe_create [(1, [1; -2]); (1, [2; -1]); (3, [3; -3])] |> List.map (fun (label, l) -> convert P.k label, l));
    Printf.printf "test 1\n";errorer (s 1 @ s 1) (unsafe_create [(1, [1; -1]); (1, [2; -2]); (3, [3; -3])] |> List.map (fun (label, l) -> convert P.k label, l)); (* a case where a label is reduced *)
    Printf.printf "test 2\n";errorer (p 1) (of_ill [[1]; [-1]; [2; -2]; [3; -3]]);
    Printf.printf "test 3\n";errorer (s 1 @ id) (unsafe_create [(1, [1; -2]); (1, [2; -1]); (3, [3; -3])] |> List.map (fun (label, l) -> convert P.k label, l));
