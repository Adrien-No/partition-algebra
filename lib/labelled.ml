...
module Utils = struct
  ...
  (* let filter f l = *)
  (*   List.map (function Few (_, l) -> if f l then  | Unique x -> Unique x) *)
  (* let filter_map f l = *)
  (*   map f l *)
  (*   |> filter ((<>)None) *)
  (*   |> map Option.get *)
...
module Make (P: sig val k : int end) = struct
  ...
  let law_mult (l : int list) : int = List.fold_left (fun x y -> min (abs x) (abs y)) P.k l
  ...
  let concat (a: t) (b: t) : t =
    ...
    let label_of_cl (cl: int list) =
      ...
      law_mult labels
    in
    let res_toobig = of_unlabelled label_of_cl c in
    let f =
      function
      | n when n <   P.k -> Some n
      | n when n < 2*P.k -> None
    ...
    in
    let rec resize l acc = (* TODO à vérifier *)
      match l with
      | [] -> acc |> Utils.sort
      | Unique x::q -> let new_x = f x in if new_x = None then resize acc q else resize q (Unique (Option.get new_x)::acc)
      | Few (label, l)::q ->
        let rec loop l acc' =
          match l with
          | [] -> if acc' = [] then resize acc q else resize q (Few(label, acc')::acc)
          | x::t ->
            let new_x = f x in if new_x = None then loop t acc' else loop t ((Option.get new_x::acc'))
        in
        loop l []
    in
    resize res_toobig []
  ...
  let generator_builder i imax (f: int -> int list list) =
    ...
    let unlabelled =
      ...
      loop [] 0
    in
    unlabelled |> of_unlabelled law_mult |> Utils.sort
...
(*   type t = cl list *)
(*   and cl = Uniq of node | Few of label * node list *)
...
(*   type label = P.label *)
(*   type t = cl list *)
(*   and cl = Uniq of P.node | Few of label * P.node list *)
